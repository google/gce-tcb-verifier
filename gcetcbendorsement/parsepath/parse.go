...
func (p *parser) accessIdent(pos int, id string) error {
	...
	p.path = append(p.path, protopath.FieldAccess(fd))
	return nil
}
...
func (p *parser) accessValue(pos int, lit forAccess) error {
	...
	if fd.IsMap() {
		...
		p.path = append(p.path, protopath.MapIndex(mk))
	} else {
		i, ok := lit.castInt()
		if !ok {
		...
		p.desc = fd.Message()
		p.path = append(p.path, protopath.ListIndex(i))
	}
...
func ParsePath(md protoreflect.MessageDescriptor, path string) (protopath.Path, error) {
	if md == nil {
