// Package parsepath provides a parser for the protopath syntax of referencing fields within a
// protobuf.
package parsepath
...
// gce-tcb-verifier:google3-begin()

// ToFuzz is an expected type for the fuzzer to continually pull tokens from.
type ToFuzz struct {
	s *scanner
}

// Step continues scanning until the end of the input.
func (t *ToFuzz) Step() {
	t.s.scan()
}

// ScanForFuzz returns a ToFuzz struct that can be used to fuzz the input.
func ScanForFuzz(in []byte) *ToFuzz {
	return &ToFuzz{s: &scanner{buf: in, pos: 0}}
}

// gce-tcb-verifier:google3-end()
